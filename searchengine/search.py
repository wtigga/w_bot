...
class crawler:
	...
	def crawl(self, pages, depth=2):
		...
		for page in pages:
			...
			print('got %s links from crawled url from %a' % (len(links), url))
			for link in links:
				if 'href' in dict(link.attrs):
					...
					if url[0:4] == 'http' and 'bkrs.info' in url and not self.isindexed(url):
						newpages.add(url)
					elif self.isindexed(url):
						print('already indexed %s' % url)
					linkText = self.gettextonly(link)
					self.addlinkref(page, url, linkText)
				self.dbcommit()
		print('got %s unindexed links from crawled url from %a' % (len(newpages), url))
		# pages = newpages
		if len(newpages) > 0:
