...
class crawler:
    ...
    def crawl(self, pages):
        ...
        for page in pages:
            ...
            for link in links:
                if 'href' in dict(link.attrs):
                    ...
                    if url[0:4] == 'http' and 'bkrs.info' in url and not isindexed:
                        newpages.add(url)
                        q = (url, link.string, "N")
                        self.con.execute('insert into urlcheck values (?, ?, ?)', q)
        ...
        if len(newpages) > 0:
            ...
            newpages = [i[0] for i in self.con.execute('select url from urlcheck where indexed = "N" order by random() limit 20')
                            .fetchall()]
            if len(newpages) > 0:
